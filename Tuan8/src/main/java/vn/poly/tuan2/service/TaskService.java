...
public class TaskService {
    ...
    public Page<TaskDto> getAllTasksForAdmin(TaskStatus status, Pageable pageable) {
        Page<Task> tasksPage;
        if (status != null) {
            tasksPage = taskRepository.findAll((Specification<Task>) (root, query, criteriaBuilder) ->
    ...
    public Optional<TaskDto> getTaskByIdForAdmin(Long id) {
        Task task = taskRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Task not found with id " + id));
        return Optional.of(convertToDto(task)); // Chuyển đổi sang TaskDto
    }
    public Page<TaskDto> getAllTasksByAdmin(TaskStatus status, Pageable pageable) {
        Page<Task> tasksPage;
        if (status != null) {
